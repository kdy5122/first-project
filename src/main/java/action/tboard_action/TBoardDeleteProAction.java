...
public class TBoardDeleteProAction implements action.Action {
	...
	public ActionForward execute(HttpServletRequest request,HttpServletResponse response) throws Exception{	 
		...
		TBoardDeleteProService tboardDeleteProService = new TBoardDeleteProService();
		...
		//isArticleWriter(DAO의 isArticeBoraderWirter 참조[select borad num해서 그 넘버에대한 비밀번호가 pass와 일치하면 isWriter true ])경로 전체를 삭제하여 확인누르면 바로삭제작동
			...
			boolean isDeleteSuccess = tboardDeleteProService.removeArticle(tboard_num);
			...
			if(!isDeleteSuccess){
				...
				forward.setPath("tboardList.bo?page=" + nowPage);
			}
