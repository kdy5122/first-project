...
public class prodInsertProAction implements Action {
		...
		/*관리자 아이디가 맞는지 세션에서 확인작업?*/
		 ...
		 File file2 = new File(realFolder); if(!file2.exists()) { file2.mkdirs(); }
			...
			*/
			...
			ProdInsertService prodInsertService = new ProdInsertService();
			boolean isAddProdSuccess = prodInsertService.registProd(productBean);
